...
//      inorderTraversal(mirrorRoot);
    ...
    /**
     * 将二叉查找树（二叉排序树）变为有序的双向链表 要求不能创建新节点，只调整指针。
     * 递归解法：
    ...
    public static Result getMaxDistanceRec(TreeNode root){
        ...
        res.maxDepth = Math.max(lmd.maxDepth+1, rmd.maxDepth+1);        // 当前最大深度
        // 取情况A和情况B中较大值
        res.maxDistance = Math.max( lmd.maxDepth+rmd.maxDepth+2, Math.max(lmd.maxDistance, rmd.maxDistance) );
        return res;
    }
